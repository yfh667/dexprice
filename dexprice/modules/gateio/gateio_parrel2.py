...
class GateTaskManager2:
    def __init__(self, namesqueues:list[queuefortoken.namequeue],    proxies, rate, capacity, max_threads_per_proxy):
        ...
        self.total_tasks = math.ceil(len(namesqueues) / self.batch_size)
    ...
    def process_task(self, namequeue:queuefortoken.namequeue):
        success = False
        failure_count = 0
        ...
        while not success and failure_count < max_failures:
            ...
            #  tokens_info = Get_Token_Dexscreen(self.sourcetype, self.chain_id, addresses, proxy.port)
                if tokens_info == "FAILED":
                    print(f"Request failed for addresses {namequeue.name}. Retrying...")
                    failure_count += 1
                    time.sleep(2)
        ...
        if not success:
            print(f"Failed to process addresses {namequeue.name} after {max_failures} attempts.")

            self.failed_tasks.put(namequeue)
        return success
