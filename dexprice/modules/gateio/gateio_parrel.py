...
class GateTaskManager:
    def __init__(self, names,   proxies, rate, capacity, max_threads_per_proxy,timeframe, aggregate, before_timestamp, limit):
        ...
        self.total_tasks = math.ceil(len(names) / self.batch_size)
    ...
    def process_task(self, name):
        ...
        while not success and failure_count < max_failures:
            ...
            try:
                ...
                # 速率限制允许，进行请求
                tokens_info =  gateprice.get_token_history2(name[0]+'_USDT', self.aggregate+self.timeframe,  self.limit, int(self.before_timestamp), proxy.port)

                if tokens_info == "FAILED":
                    print(f"Request failed for addresses {name}. Retrying...")
                    failure_count += 1
                    time.sleep(2)
        ...
        if not success:
            print(f"Failed to process addresses {name} after {max_failures} attempts.")
            for address in name:
