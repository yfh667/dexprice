...
from dexprice.modules.PriceMonitor.dexscreen_priceapi import Get_Token_Dexscreen
...
class TaskManager:
    def __init__(self, addresses, sourcetype:define.Config, chain_id, proxies, rate, capacity, max_threads_per_proxy,mark=''):
        self.task_queue = Queue()
        self.failed_tasks = Queue()
        self.results = []
        self.result_lock = threading.Lock()
        self.batch_size = 30
        if(sourcetype == define.Config.DEXS):
            self.batch_size = 30
        elif(sourcetype == define.Config.DEXCA):
            self.batch_size = 1
      ...
      #  self.proxy_pool.add_proxy()
        ...
        self.progress_bar = tqdm(total=self.total_tasks, desc=mark+"Total Progress")
    ...
    def run(self):
      ...
      #  max_retries = 0
        ...
        retries = 0
        # 主线程监控代理池和线程数量
        network_check_interval = 10  # 每隔多少秒检查一次网络
        ...
        while True:
            ...
            if time.time() - last_network_check >= network_check_interval:
                last_network_check = time.time()
                if not testnetwork.test_baidu():
                    print("Network is down. Exiting loop early.")
                    break
            if self.task_queue.empty():
            # 检查失败任务队列是否为空
            ...
            with self.proxy_pool.lock:
                ...
                if(total_capacity == 0):
                    print("no avaliable proxies, network is down  we break the task")
                    break
