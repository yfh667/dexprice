...
class MexcTaskManager:
    def __init__(self, symbollist:list[str], proxies, rate, capacity, max_threads_per_proxy):
        ...
        self.total_tasks = math.ceil(len(symbollist) / self.batch_size)
        self.progress_bar = tqdm(total=self.total_tasks, desc="MEXC_Total Progress")

    def add_tasks(self, symbollist):
        #  batch_size = 30
    ...
    def process_task(self, symbol):
        ...
        if not success:
            print(f"Failed to process addresses {symbol} after {max_failures} attempts.")

            self.failed_tasks.put(symbol)
        return success
