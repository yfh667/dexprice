...
class MexcOvhlTaskManager:
    ...
    def process_task(self, queue):
        ...
        if not success:
            print(f"Failed to process addresses {queue} after {max_failures} attempts.")

            self.failed_tasks.put(queue)
        return success
