...
def get_token_history(ohlcv_data ):
    historydatas = []
    for entry in ohlcv_data['data']['attributes']['ohlcv_list']:

        historydata =  define.TokenPriceHistory(1,entry[1],entry[2],entry[3],entry[4],timestamp_to_datetime(entry[0]),entry[5])
        # print(entry)
...
# 示例用法
network = "solana"
pool_address = "G5ts2NDTcAhzowLqWTrVN6NcxKoAwrXHg3uPTyskfksd"
...
historydatas = get_token_history(ohlcv_data)
print(historydatas)
