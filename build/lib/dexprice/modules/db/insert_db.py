...
class DatabaseInterface(ABC):
    ...
    def getpricedexscreen(self,  tokenid: int):
        pass
    # @abstractmethod
    # def createpricedaily(self,table_name,chainid,pairaddress):
    #     pass
    @abstractmethod
    def connect(self):
        pass
    ...
    def insert_data(self, table_name,name, ca, pairaddress, creattime):
        pass
    def insert_tokeninfo(self, table_name, token: 'TokenInfo'):
        """
        插入 TokenInfo 数据到数据库中
        """
        # 检查 token 是否为空
        if not token:
            print("TokenInfo is empty, skipping insert.")
            return
    ...
    def insert_multiple_tokeninfo(self, tokens: list[define.TokenInfo]):
        ...
        table_name = "token_pairs"
        for token in tokens:
            self.insert_tokeninfo(table_name, token)
...
#             print(f"Database '{self.db_path}' does not exist. Creating and initializing the database.")
...
#         self.conn.commit()
    ...
    def insert_multiple_price_history(self, token_price_history_list):
        ...
        self.conn.commit()
        print(f"Batch inserted {len(data_to_insert)} records into {table_name}.")
    ...
    def FindParetokenid(self, pairaddress:str):
      ...
      #  pa = pairaddress
        ...
        # 查询 token_pairs 表中与 CA 匹配的 id
        query = f"SELECT id FROM {table_name} WHERE pairaddress = ?"
    ...
    name TEXT NOT NULL,
    ca TEXT NOT NULL UNIQUE,  -- 添加 UNIQUE 约束
